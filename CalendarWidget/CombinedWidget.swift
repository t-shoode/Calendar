...
struct CombinedEntry: TimelineEntry {
  ...
  let monobankBalances: [WidgetMonobankBalanceItem]
...
private struct MonobankCurrencyTotal: Identifiable {
  let currency: String
  let totalMajor: Double

  var id: String { currency.lowercased() }
}

private struct MonobankCurrencyTrend: Identifiable {
  let currency: String
  let delta7Major: Double
  let delta30Major: Double
...
struct CombinedProvider: TimelineProvider {
  ...
  private func createEntry(for date: Date) -> CombinedEntry {
    ...
    let monobankAuthorized = defaults.bool(forKey: "widgetMonobankAuthorized")
    ...
    let timerEndTime =
      defaults.object(forKey: "widget_\(activeTimerId ?? "default")_timerEnd") as? Date
    let stopwatchStartTime =
      defaults.object(forKey: "widget_\(activeTimerId ?? "default")_stopwatchStart") as? Date
    ...
    let monobankBalances = monobankAuthorized ? loadMonobankBalances(from: defaults) : []
    ...
    return CombinedEntry(
      ...
      monobankBalances: monobankBalances,
      forcedColorScheme: forcedScheme
    )
  }

  private func loadMonobankBalances(from defaults: UserDefaults) -> [WidgetMonobankBalanceItem] {
    guard let data = defaults.data(forKey: "widgetMonobankBalances"),
      let balances = try? JSONDecoder().decode([WidgetMonobankBalanceItem].self, from: data)
    else {
      return []
    }
    return balances
...
struct CombinedWidgetEntryView: View {
  ...
  private var scheme: WidgetColorScheme {
    WidgetColorScheme.from(
      forcedColorScheme: entry.forcedColorScheme, environment: systemColorScheme)
  }

  private var monobankCurrencyTotals: [MonobankCurrencyTotal] {
    var totalsByCurrency: [String: Double] = [:]
    for balance in entry.monobankBalances {
      totalsByCurrency[balance.currency, default: 0] += balance.balanceMajor
    }

    return totalsByCurrency.keys.sorted().map { currency in
      MonobankCurrencyTotal(
        currency: currency,
        totalMajor: totalsByCurrency[currency, default: 0]
      )
    }
  }

  private var monobankCurrencyTrends: [MonobankCurrencyTrend] {
    var trendsByCurrency: [String: (d7: Double, d30: Double)] = [:]
    for balance in entry.monobankBalances {
      let current = trendsByCurrency[balance.currency] ?? (0, 0)
      trendsByCurrency[balance.currency] = (
        d7: current.d7 + (balance.delta7Major ?? 0),
        d30: current.d30 + (balance.delta30Major ?? 0)
      )
    }

    return trendsByCurrency.keys.sorted().map { currency in
      let trend = trendsByCurrency[currency] ?? (0, 0)
      return MonobankCurrencyTrend(
        currency: currency,
        delta7Major: trend.d7,
        delta30Major: trend.d30
      )
    }
  }

  private var firstTodoItemId: String? {
    entry.upcomingItems.first(where: { $0.type == .todo })?.id
  }

  private func quickActionURL(action: String, id: String? = nil) -> URL? {
    var components = URLComponents()
    components.scheme = "calendarapp"
    components.host = "widget-action"
  ...
  var body: some View {
    VStack(spacing: 0) {
      ...
      if !entry.monobankBalances.isEmpty {
        VStack(alignment: .leading, spacing: 4) {
          HStack(spacing: 6) {
            ...
            ForEach(monobankCurrencyTotals) { total in
              Text(
                "\(currencySymbol(total.currency))\(Int(total.totalMajor))"
              )
          ...
          if !monobankCurrencyTrends.isEmpty {
            HStack(spacing: 8) {
              ForEach(monobankCurrencyTrends) { trend in
                Text(
                  "7d \(signedAmount(trend.delta7Major, currency: trend.currency)) • 30d \(signedAmount(trend.delta30Major, currency: trend.currency))"
                )
      ...
      HStack(spacing: 8) {
        if let quickAddURL = quickActionURL(action: "quickAddExpense") {
        ...
        if let openBankURL = quickActionURL(action: "openPinnedBankCard") {
          Link(destination: openBankURL) {
        ...
        if let firstTodoItemId,
          let markTodoURL = quickActionURL(action: "markTodoDone", id: firstTodoItemId)
        {
          Link(destination: markTodoURL) {
      ...
      VStack(spacing: 8) {
        ...
        CombinedWeatherWeekRow(
          week: entry.thisWeek, forecastDays: entry.forecastDays, scheme: scheme)

        // Next week with weather
        CombinedWeatherWeekRow(
          week: entry.nextWeek, forecastDays: entry.forecastDays, scheme: scheme)
      }
      .padding(.horizontal, 12)
  ...
  private func signedAmount(_ value: Double, currency: String) -> String {
    let sign = value >= 0 ? "+" : "-"
    return "\(sign)\(currencySymbol(currency))\(Int(abs(value)))"
  }
}
